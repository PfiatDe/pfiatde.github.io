---
layout: post
title:  "How your messenger used for internal communication might compromise your network"
date:   2023-02-08 18:44:17 +0200
author: PfiatDe
excerpt_separator: <!--more-->
---

In this blog post some techniques about the messengers Mircrosoft Skype-for-Business (S4B) and Microsoft Teams regarding attacking a company network are shown.

Nothing fancy new here, just some well-known techniques, which work too often and companies are not aware off. The success rate for this kind of phishing / socail engineering is very high.
...
* If an alias address, in the form `<<ADUSER>>.company.org`, Teams and S4B will happily resolve the AD-User to First and Last Name
...
S4B can often detected via a subdomain called lyncdiscover.company.org. If the domain exist, it will typicall return a XML file, telling where the S4B Server can be found.
...
If you follow the link to the S4B server you may get a `403 - Forbidden: Access is denied.`. You can still identify the S4B by the favicon, or you could just try the `/scheduler` endpoint. This brings up a wonderful login page with internal AD credentials.
...
![Pew pew pew](/assets/media/S4B/futurama-fry.gif){:width="100%"} *Pew pew pew*
...
![Skype scheduler](/assets/media/S4B/Skype_Schedulerpng){:width="100%"} *Skype scheduler*
...
## Testtenant
To attack such a setting, we need an AAD with an Teams account. The free version of Teams, does work for enumeration, but will likely not work for communication or show at least more warnings.

Microsoft offers a possiblities to register a development AAD. [https://developer.microsoft.com/en-us/microsoft-365/dev-program](https://developer.microsoft.com/en-us/microsoft-365/dev-program)

![Developer-AAD](/assets/media/S4B/Developer-AAD.png){:width="100%"} *Developer-AAD* 
...
But even if not, we still can verify E-Mails against the company, as Teams will return a different message if the user exists. ![Developer-AAD](/assets/media/S4B/Teams_Userenumeration.png){:width="100%"} *User Enumeration* 
...
With our Developer AAD, we can add users like we want to. For example we can add some user called ithelpdesk@company.com as nickname. Unfortunally we can not simply improve this, by adding a generic company logo as profile picture, as the profile picture is not shared across the tenants.
...
S4B / Teams does also not offer protection against spoofing things in messages. For example a simpe HTML spoofing is possible. Meaning the text and the link are pointing to different sites.
By looking at the requests causing a new message to send it is quite obvious how to change the tags.
![Developer-AAD](/assets/media/S4B/Teams_Spoof_2.png){:width="100%"} *Showing the Identity* 

By doing this, it is possible to craft messages, which open a malicious URL despite showing a company owned URL.
![Developer-AAD](/assets/media/S4B/Teams_Spoof_1.png){:width="100%"} *Showing the Identity* 
...
To further provide some data about the possible risk the DAX-Companies have been quickly analysed.
...
![Developer-AAD](/assets/media/S4B/Enum_Emails.png){:width="100%"} *Showing the Identity* 
...
$dax = Import-Csv D:\Nextcloud\Notes\Blog\Skype\dax.csv
...
![Developer-AAD](/assets/media/S4B/Teams_Settings_6.png){:width="100%"} *Showing the Identity* 
...
There is no protection against the Teams E-Mail enumeration, beside not to use Teams. However, company E-Mail addresses should not be considered as secret, as there are several ways to gather and verify them.
...
![Developer-AAD](/assets/media/S4B/Teams_External_Settings.png){:width="100%"} *Teams settings for external communication* 
